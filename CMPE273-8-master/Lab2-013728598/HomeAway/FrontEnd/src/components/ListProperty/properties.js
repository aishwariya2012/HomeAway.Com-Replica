...
import {Redirect} from 'react-router';
 ...
 class properties extends Component{
    ...
    render(){
        ...
        if(this.props.Address===true){
            redirect = <Redirect to= "/ListDetails"/>   
        }
        return(
            <div>
                {redirect}
...
const mapStateToProps = state => { 
    console.log("Value of properties",state);
    return {
        ...
        onSubmitHandle : (data) => {
            ...
            axios.post('http://localhost:3001/properties', data)
                .then((response) => {
                    ...
                    dispatch({type: 'Property',payload : response.data.updatedList,statusCode : response.status})
            });
        }
    }
}
export default connect(mapStateToProps,mapDispatchStateToProps)(properties); 