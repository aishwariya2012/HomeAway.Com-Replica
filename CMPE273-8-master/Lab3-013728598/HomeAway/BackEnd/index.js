...
app.post('/SignUpEmail', function (request, response) {
    ...
    if (!request.body.username || !request.body.password) {
        ...
        db.createUser(newUser, function (res) {
            ...
            return response.status(200).json({success: true, message: 'username address done',AccountCreated:true});
        }, function (err) {
            console.log(err);
...
app.post('/Traveller', function (request, response) {
   console.log("Inside Login Request and Parameters Are",request.body);
    ...
    db.findUser({
        ...
        crypt.compareHash(request.body.password,res.PasswordEncrypt , function (err, isMatch) {
            if (isMatch && !err) {
                ...
                response.status(200).json({success: true,token: 'JWT ' + token,resData,message:'Logged in Succesfully'});
...
app.post('/HomeDisplay',function(req,res){
    console.log("Inside Search Request:",req.body);
    PropertyData.find({
        City:req.body.Search,
        //Accomodates: { $gte: req.body.Guest },
        //AvailableStartDate:{$lte:req.body.Start},
        //AvailableEndDate:{$gte:req.body.End}
        ...
        const propertydata={
            Properties:docs,
            SearchParameter:req.body
        }
       res.end(JSON.stringify(propertydata));
       console.log(docs);
    },(err) => {
        console.log(err);
        res.code = "400";
